/*
  VescThrottle_GT911_BBCapTouch_StandaloneTx.ino
  - KEINE Änderungen an VescBLE.cpp
  - Classic BLE scan/connect, bindet vescblerxtx via set_writer()
  - bb_captouch (GT911) + TFT_eSPI 240x320 Portrait
*/
...
static bool touch_point(int &tx, int &ty){
  ...
  if (ti.count<=0) return false;
  int x=ti.x[0], y=ti.y[0];
  if (TOUCH_SWAP_XY){ int t=x; x=y; y=t; }
  if (TOUCH_FLIP_X)  x = TP_W-1-x;
  if (TOUCH_FLIP_Y)  y = TP_H-1-y;
  if (x<0) x=0; if (x>=TP_W) x=TP_W-1;
  if (y<0) y=0; if (y>=TP_H) y=TP_H-1;
